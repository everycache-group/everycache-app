...
from everycache_api.extensions import apispec, jwt, pwd_context
...
def login():
    ...
    if email_address is None or password is None:
        return jsonify({"msg": "Missing e-mail address or password"}), 400

    user = User.query.filter_by(email=email_address).first()
    if user is None or not user.verify_password(password):
        return jsonify({"msg": "Incorrect email and password combination"}), 400
