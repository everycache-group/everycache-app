...
class UserSchema(ma.SQLAlchemyAutoSchema):
    ...
    password = ma.String(load_only=True, required=True, validate=validate.Length(8, 255))
    ...
    def validate_username(self, value):
        def _validate_username_three_letters(value):
            if sum((value.count(let) for let in ascii_letters if let in value)) < 3:
                raise ValidationError("The username must contain at least three letters.")

        return validate.And(
            validate.ContainsOnly(
                ascii_letters + "_" + digits,
                error="The username can consist only of letters, digits and '_'."),
